...
def update(dt,):
	global rgbLedValues,label,knobValues

	knobValues[0] = knobs[0].read()
	knobValues[1] = knobs[1].read()
	knobValues[2] = knobs[2].read()
	
	pyglet.gl.glClearColor(knobValues[0],knobValues[1],knobValues[2],1.)
	label.text = "({:.2f},{:.2f},{:.2f})".format(knobValues[0],knobValues[1],knobValues[2])

	wek_send_message("/wek/inputs",knobValues)
